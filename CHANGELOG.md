...
- **Configuration init and loading**: When setting up config and loading it, the project name is normalized so if there's space in the name, the correct custom prompt can still be found.
...
### Fixed
- **Review State Pruning**: Optimized the review state pruning logic to improve performance and accuracy.
  - Added a check for `updated_at` timestamp to ensure that the pruning logic only removes outdated entries.
  - This prevents the removal of valid review states and improves the overall reliability of the auto-review feature.
...
### Fixed
- **Review State File Organization**: Fixed review state file path to be consistent with other configuration files
  - Changed review state file location from `~/.aicodereview_state.json` to `~/.aicodereview/review-state.json`
  - Ensures all AI Code Review files are organized in the `~/.aicodereview/` directory
  - Added automatic directory creation if `.aicodereview` directory doesn't exist
