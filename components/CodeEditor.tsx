    ...
    <div className="bg-white dark:bg-brand-surface rounded-lg shadow-xl h-full flex flex-col">
       <div className="p-4 border-b border-gray-200 dark:border-brand-primary flex justify-between items-center">
        ...
        <div className="relative" ref={filterRef}>
            <button onClick={() => setIsFilterOpen(!isFilterOpen)} className="flex items-center space-x-2 text-xs bg-gray-100 dark:bg-brand-primary hover:bg-gray-200 dark:hover:bg-brand-secondary text-gray-800 dark:text-white font-semibold py-2 px-3 rounded-md transition-colors">
                <FilterIcon />
                        ...
                        {isLoadingProjects ? <div className="p-4 text-center"><Spinner size="md" /></div> :
                            filteredProjects.map(project => (
                                <label key={project.id} className="flex items-center space-x-3 p-2 hover:bg-gray-100 dark:hover:bg-brand-primary rounded-md cursor-pointer">
                                    ...
                                    <span className="text-xs text-gray-800 dark:text-brand-text">{project.name_with_namespace}</span>
                                </label>
                            ))
                        }
                    </div>
      ...
      <div className="p-4 border-b border-gray-200 dark:border-brand-primary">
          <form onSubmit={handleUrlSubmit}>
              <label htmlFor="mr-url-input" className="block text-xs font-medium text-gray-600 dark:text-brand-subtle mb-2">
                  Review by URL
              </label>
      ...
      <div className="flex-grow overflow-y-auto">
        {isLoading ? (
            <div className="flex flex-col items-center justify-center h-full text-center p-4">
                <Spinner size="lg" />
                <p className="mt-4 text-sm text-gray-500 dark:text-brand-subtle">Analyzing Merge Request...</p>
            </div>
        ) : (
            <div className="p-4">
                {error && <div className="text-center text-red-600 dark:text-red-400 bg-red-100 dark:bg-red-900/30 p-4 rounded-lg mb-4"><h3 className="font-bold text-sm">An Error Occurred</h3><p className="text-xs">{error}</p></div>}
                <h3 className="text-sm font-semibold text-gray-500 dark:text-brand-subtle mb-3">Or Select from List</h3>
                        ...
                        <ul className="space-y-3">
                            {mergeRequests.map(mr => (
                                <li key={mr.web_url} onClick={() => onReview(mr.web_url)} className="bg-gray-100/50 dark:bg-brand-primary/40 hover:bg-gray-100 dark:hover:bg-brand-primary/80 p-4 rounded-lg cursor-pointer transition-colors group">
                                    <p className="font-semibold text-gray-800 dark:text-brand-text group-hover:text-brand-secondary dark:group-hover:text-brand-secondary transition-colors text-sm">{mr.title}</p>
                                    <div className="text-xs text-gray-500 dark:text-brand-subtle mt-2 flex items-center justify-between">
                                        <div className='truncate pr-2'>
                                            <span className="text-xs">!{mr.iid} in {mr.project_name} by @{mr.author.name}</span>
                                        </div>
